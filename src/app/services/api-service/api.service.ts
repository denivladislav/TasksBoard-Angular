...
import { delay, MonoTypeOperatorFunction, Observable } from 'rxjs';
...
export class ApiService {
    private _baseUrl: URL = new URL('http://localhost:3000/items');
    private _http = inject(HttpClient);
    // Slowing down requests in order to demonstrate project features
    private _customDelay<T>(due = 500): MonoTypeOperatorFunction<T> {
        return delay<T>(due);
    }
    ...
    patchItem(payload: Required<Pick<ToDoListItem, 'id'>> & Partial<ToDoListItem>): Observable<ToDoListItem> {
    ...
    deleteItem(payload: Pick<ToDoListItem, 'id'>): Observable<ToDoListItem> {
        const url: URL = new URL(this._baseUrl);
        url.pathname = [url.pathname, payload.id].join('/');
